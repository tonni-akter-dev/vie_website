import React from 'react';
const faqData = [
  "What service does VIE offer ?",
  "Who can apply through VIE Service ?",
  "Do you need to know which course or university I want before contacting you?",
  "How long does the application  process take?",
  "Is VIE Services affiliated with Australian universities?",
  "Do you charge for your services?",
  "Can VIE Services help with scholarship?",
  "What support do you provide after the visa granted?",
  "What if my visa is refused?",
  "Can I work while studying?",
  "How much does it cost to study in Australia?",
  "How early should I start my application process?",
  "What is a confirmation of enrolment (COE)?",
  "What are the English language requirements?",
  "Why should I choose VIE Services over other Consultants?"
];

const FaqItem = ({ question, index }: { question: string, index: number }) => {
  const isEven = index % 2 === 0;
  return (
    <div className='flex flex-col items-center justify-center mb-8'>
      <div className={`${isEven ? 'bg-[#FFF2DE] hover:bg-[#EEDEC7]' : 'bg-[#EEDEC7] hover:bg-[#FFF2DE]'
        } cursor-pointer transition-all ease-in-out duration-300 rounded-[22px] h-[111px] flex justify-center items-center w-full text-center`} style={{ boxShadow: '0 1px 4px 0 rgba(0, 0, 0, 0.25)' }}>
        <p className='text-[#533115] text-[28px] font-medium'>{question}</p>
      </div>
      <div className='mt-[-10px]'>
        <svg xmlns="http://www.w3.org/2000/svg" width="41" height="41" viewBox="0 0 41 41" fill="none">
          <circle cx="20.5" cy="20.5" r="20.5" fill="#533115" />
          <path d="M29.75 17.375H23.25V11.125C23.25 10.2962 22.9076 9.50134 22.2981 8.91529C21.6886 8.32924 20.862 8 20 8C19.138 8 18.3114 8.32924 17.7019 8.91529C17.0924 9.50134 16.75 10.2962 16.75 11.125L16.8654 17.375H10.25C9.38805 17.375 8.5614 17.7042 7.9519 18.2903C7.34241 18.8763 7 19.6712 7 20.5C7 21.3288 7.34241 22.1237 7.9519 22.7097C8.5614 23.2958 9.38805 23.625 10.25 23.625L16.8654 23.5141L16.75 29.875C16.75 30.7038 17.0924 31.4987 17.7019 32.0847C18.3114 32.6708 19.138 33 20 33C20.862 33 21.6886 32.6708 22.2981 32.0847C22.9076 31.4987 23.25 30.7038 23.25 29.875V23.5141L29.75 23.625C30.612 23.625 31.4386 23.2958 32.0481 22.7097C32.6576 22.1237 33 21.3288 33 20.5C33 19.6712 32.6576 18.8763 32.0481 18.2903C31.4386 17.7042 30.612 17.375 29.75 17.375Z" fill="white" />
        </svg>
      </div>
    </div>
  );
};

const FaqList = () => {
  return (
    <div className='lg:mx-[152px] mb-[156px]'>
      <h2 className='uppercase text-[#533115] text-center text-[36px] lg:text-[64px] font-[900] leading-normal mb-10'>frequently asked questions</h2>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mt-[58px]'>
        {faqData.map((faq, index) => (
          <FaqItem key={index} question={faq} index={index} />
        ))}
      </div>
    </div>
  );
};

export default FaqList;